...
function getFileContent(fpath, opts) {
    ...
    return prepare(fpath, cont, opts)
}
...
function prepare(fpath, text, opts) {
    return cache(fpath, text, (doCache) => {
        const moduleText = rDefine.test(text) ? text : `define(function(require, exports, module) { \n${text}\n })`
        const dstText = opts.useES6 ? es6(moduleText) : moduleText
        doCache(dstText)
        return dstText
    })
}
...
module.exports = function(file, opts) {
    var data = prepare(file.path, file.contents.toString('utf8'), opts)
